...
class ContaVeProApp {
  ...
  getStartUrl() {
    ...
    console.log('Available paths in __dirname:', fs.readdirSync(__dirname));
    console.log('Available paths in parent:', fs.readdirSync(path.join(__dirname, '..')));
  ...
  initializeDatabase() {
    const userDataPath = app.getPath('userData');
    const dbPath = path.join(userDataPath, 'contave.db');
    
    // Initialize database schema if it doesn't exist
    if (!fs.existsSync(dbPath)) {
      this.createInitialDatabase(dbPath);
    }
  }

  createInitialDatabase(dbPath) {
    try {
      // Copy schema from resources to userData
      const schemaPath = path.join(__dirname, '..', 'database', 'schema.sql');
      if (fs.existsSync(schemaPath)) {
        const schema = fs.readFileSync(schemaPath, 'utf8');
        // Initialize with schema (would require better-sqlite3 here)
  ...
  async handleBackupDatabase() {
    try {
      const userDataPath = app.getPath('userData');
      const sourceDb = path.join(userDataPath, 'contave.db');
